...
<script type="text/javascript">
   ...
   var components = {
       label: {
           ...
           template: '<span><label :for="id" v-if="label != \\'\\'">{{label || ' + fancyName('id') + '}} <i v-show="description && !hide">({{description}})</i></label><a href="#" v-on:click.prevent.stop="touch=true">[open]</a></span>'
       },
       text:  {
       ...
       thintext:  {
           ...
           template: '<div><mip-label :id="id" :label="label" :description="description" :hide="!touch"></mip-label><input type="text" :name="id" v-if="value || touch" :placeholder="placeholder" v-bind:value="value" v-on:input="$emit(\\'update:value\\', $event.target.value);" :disabled="disabled == \\'true\\'" :readonly="readonly == \\'true\\'"></div>'
       },
       hide:  {
