...
# A PATCH request.
#
class Mushy::Patch < Mushy::Flux
  def self.details
    {
      name: 'Patch',
      title: 'Make a PATCH call',
      fluxGroup: { name: 'Web' },
      description: 'Makes a PATCH call to a URL',
      config: {
        url: {
      ...
      examples: {
        'Successful Call' => {
          description: 'This will send a PATCH to https://www.google.com.',
  ...
  def process(_event, config) # rubocop:disable Metrics/MethodLength
    ...
    response = faraday.patch(config[:url]) do |request|
