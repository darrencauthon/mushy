...
# A PUT request.
#
class Mushy::Put < Mushy::Flux
  def self.details
    {
      name: 'Put',
      title: 'Make a PUT call',
      fluxGroup: { name: 'Web' },
      description: 'Makes a PUT call to a URL',
      config: {
        url: {
      ...
      examples: {
        'Successful Call' => {
          description: 'This will send a PUT to https://www.google.com.',
  ...
  def process(_event, config) # rubocop:disable Metrics/MethodLength
    ...
    response = faraday.put(config[:url]) do |request|
